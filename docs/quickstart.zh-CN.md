...
1. **找能力：** 在 [Skill Atlas](https://sandbaseai.github.io/workbuddy-skill/) 搜索任务词；需要能直接导入的结果时，打开“有精选包可用”。
2. **看清楚：** 打开来源链接，确认许可证、版本、需要的连接器、权限和可能的写入/付费/发消息行为。
3. **先试用：** 下载同一 Release 的 ZIP 和 `SHA256SUMS`，校验后把原始 ZIP 导入 WorkBuddy；第一次只用公开或测试数据，并先发送下面的只读提示词。

```text
先说明你将使用的能力、输入、权限、外部副作用和预计成本。
现在只做只读检查：不要调用付费 API，不要修改数据，不要发送消息。
如果缺少权限、连接器或参数，请先列出缺项，不要猜测或继续执行。
```

如果你还没有确定要找什么，直接看 [Starter Packs 入门包](starter-packs.zh-CN.md)；如果你只想使用现成包，跳到[5 分钟安装](#5-分钟安装一个精选-skill)；如果导入失败，先看文末的[常见问题](#常见问题)。
...
| 在本地搜索目录 | 执行 `python3 scripts/query_catalog.py <关键词> --installable --limit 10` 快速找可下载精选包；也可以使用 `--package-status reviewed` 和 `--category 研究` 等筛选。 |
...
如果想按官方顺序认识产品，可先看[WorkBuddy 产品简介](https://www.workbuddy.ai/docs/zh/)；需要管理任务列表、并行任务、任务对话和结果面板时，再看[Agents 快速开始](https://www.workbuddy.ai/docs/zh/ide/User-guide/Agent-Mode/Quickstart)。
...
| 你的需求 | 模式 | 第一步 |
|---|---|---|
| 提问或查看信息 | **问一问（Ask）** | 先要求只读回答或文件清单 |
| 执行前审阅多步骤方案 | **想一想（Plan）** | 要求列出范围、文件、副作用和停止条件 |
| 执行可信且可回滚的任务 | **做一做（Craft）** | 使用可恢复工作区，并先检查第一份结果 |

不确定时先用 Ask 或 Plan；工作模式和权限模式是两个独立设置。
...
1. 在 [Skill Atlas](https://sandbaseai.github.io/workbuddy-skill/) 或[中文目录](https://sandbaseai.github.io/workbuddy-skill/zh-CN.html)搜索任务、能力、类别或审阅信号，例如 `ocr`、`web search`、`research`、`no-static-flags`。如果想直接找可安装的精选包，先把 **WorkBuddy 包状态 → 有精选包可用** 打开。
