...
- [Verified Computation](https://github.com/heat-death/verified-computation)——MIT 许可的可移植精确计算 Skill，要求把数学计算交给工具、把解释留给模型；适配时检查工具实现、数值边界和输入来源。
- [skill-mcp](https://github.com/tsumon/skill-mcp)——MIT 许可的本地 stdio MCP Skill 路由器与评测夹具，强调限制上下文数量和 token 预算；连接 WorkBuddy 前检查本地进程、读取范围和 Skill 冲突处理。
...
- [中文 WorkBuddy Skills 增强集](https://github.com/dingdong905/workbuddy-skills)——MIT 许可的 WorkBuddy Skill 集合，覆盖研究、记忆、上下文、股票分析和学习；README 明确记录了 WorkBuddy 专用 frontmatter 以及与 Codex 校验的边界，适配时要保留这些宿主假设。
...
- [Agent-Ready Web Profile](https://github.com/dkharlanau/agent-ready-web-profile)——Apache-2.0 许可的互操作与证据参考，覆盖 `llms.txt`、Agent Skills、MCP、A2A、Resolver 和来源证明；可用于形成可发现性改进假设，但不能把它当作网站或来源可信的证明。
- [WorkBuddy Skin](https://github.com/itcastWsy/workbuddy-skin)——MIT 许可、跨平台且可还原的界面主题参考，通过本机 CDP 注入实现，不修改官方安装文件；它不是官方产品，可能持久化调试端口，使用前应检查可执行文件、端口暴露和回滚路径。
- [WorkBuddy Skin Studio](https://github.com/cdredfox/workbuddy-skin-studio)——MIT 许可的较新非官方换肤工作流，使用本机回环 CDP；README 含自动化指令，因此只作为高风险外部参考，运行前必须人工检查脚本、调试端口暴露和回滚方式。
...
- [Skills Hub](https://github.com/qufei1993/skills-hub)——MIT 许可的跨平台 Skill 管理器，可整理并同步到多个 Agent 目录，包括 `~/.workbuddy/skills`；使用前检查扫描路径、本地设置和 Token 边界。
