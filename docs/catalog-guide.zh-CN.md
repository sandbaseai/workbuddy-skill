...
| 你的目标 | 下一步 |
|---|---|
| 现在就安装 | 打开“有精选包可用”筛选，再使用对应 Release ZIP 和校验和。 |
| 复用公开来源 | 打开固定提交来源，确认许可证和权限，再阅读[适配教程](adapting-skills.zh-CN.md)。 |
| 只做比较或研究 | 把条目当作目录参考，不要直接安装，也不要把评分当成安全结论。 |

不确定时，先走第三条路径，检查来源后再决定安装范围。
...
- **Source order**是确定性的：结果先按仓库、再按路径排序，不依赖爬取顺序，便于复查和分享同一个查询。
...
仓库的定时 `Refresh skill catalog` 工作流也会对上游来源清单中声明的仓库执行只读探测。
