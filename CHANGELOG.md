...
- Add a shareable, bilingual category directory that routes users into stable Atlas filters without adding catalog records.
...
- Keep the explicit adapter license declaration authoritative in generated Skill metadata.
- Reject adapter runs when source frontmatter disagrees with the declared source license.
...
- Added bilingual task-first Starter Pack links to the public README for faster onboarding.
- Added explicit time limits to validation, release, Pages, catalog, and branch-cleanup jobs so stalled automation cannot block delivery indefinitely.
...
- Added Issue-template routing links for quickstart help, community discussions, and private vulnerability reports.
- Added a rights-aware WorkBuddy Harness architecture and evaluation reference to both resource maps without copying its contents.
- Made the release verifier reject extra WorkBuddy ZIPs that are absent from `SHA256SUMS`.
...
- Added a post-upload Release check that confirms every generated ZIP and `SHA256SUMS` is present before the workflow succeeds.
