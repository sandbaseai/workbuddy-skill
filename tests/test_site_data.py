...
class SiteDataTests(unittest.TestCase):
    ...
    def test_curated_adaptations_are_installable_and_workbuddy_ready(self):
        ...
        self.assertIn("copy-package-link", package_page)
        self.assertIn('class="copy-package-link" hidden', package_page)
        self.assertIn('class="copy-command" aria-label=', package_page)
        self.assertIn("copyLink.hidden = false", package_page)
        self.assertIn("button.hidden = false", package_page)
        self.assertIn("Link copied / 链接已复制", package_page)
        self.assertIn("await copyText(location.href)", package_page)
        ...
        self.assertIn("params.set('category', categoryInput.value)", package_page)
        self.assertIn("item.dataset.category === category", package_page)
        ...
        self.assertEqual(package_page.count('aria-label="Inspect pinned source for '), len(packages))
        self.assertEqual(package_page.count('aria-label="Download ZIP for '), len(packages))
        self.assertEqual(package_page.count('aria-label="Verify SHA256 for '), len(packages))
        ...
        self.assertIn("const copyText = async (text)", package_page)
