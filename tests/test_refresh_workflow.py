...
class RefreshWorkflowTests(unittest.TestCase):
    def test_refresh_workflow_probes_upstream_sources_without_publishing(self):
        ...
        self.assertIn("--max-rate-wait 30", workflow)
        ...
        self.assertIn("--target 50000", workflow)
        self.assertIn("source_count=$(grep -Ev", workflow)
        self.assertIn("max_requests=$((source_count * 3 + 50))", workflow)
        ...
        self.assertIn("Manifest snapshot:", workflow)
        self.assertIn("manifest_count = sum(", workflow)
        self.assertIn("manifest_sha = hashlib.sha256(", workflow)
    ...
    def test_long_running_jobs_have_explicit_timeouts(self):
        expected = {
            ...
            ".github/workflows/refresh-catalog.yml": "timeout-minutes: 70",
