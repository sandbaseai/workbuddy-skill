...
class ValidateSiteDataTests(unittest.TestCase):
    def setUp(self):
        ...
        self.metadata = json.loads((ROOT / "site/catalog-meta.json").read_text(encoding="utf-8"))
        self.metadata_schema = json.loads((ROOT / "site/catalog-meta-schema.json").read_text(encoding="utf-8"))
    ...
    def test_current_catalog_metadata_matches_schema_contract(self):
        self.assertEqual(validate_metadata(self.metadata, self.metadata_schema), [])

    def test_rejects_mutable_or_malformed_catalog_metadata(self):
        metadata = dict(self.metadata)
        metadata["snapshot_frozen"] = False
        metadata["catalog_sha256"] = "bad"
        errors = validate_metadata(metadata, self.metadata_schema)
        self.assertIn("metadata.snapshot_frozen must be true", errors)
        self.assertIn("metadata.catalog_sha256 must be a 64-character lowercase hex SHA-256", errors)
