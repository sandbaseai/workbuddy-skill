...
class AtlasFrontendTests(unittest.TestCase):
    ...
    def test_both_languages_expose_shareable_result_controls(self):
        for page in (self.english, self.chinese):
            ...
            self.assertIn('id="installable"', page)
    ...
    def test_url_state_supports_sharing_and_browser_history(self):
        ...
        self.assertIn("function searchableText(skill)", self.app)
        self.assertIn("_searchText: searchableText(skill)", self.app)
        self.assertIn("terms.every((term) => skill._searchText.includes(term))", self.app)
        ...
        self.assertIn("controller.abort()", self.app)
        self.assertIn("signal: controller.signal", self.app)
        self.assertIn("clearTimeout(timeout)", self.app)
    ...
    def test_search_input_is_debounced_for_large_catalog_filtering(self):
        ...
        self.assertIn("skill.g", self.app)
        self.assertIn("skill.w", self.app)
        self.assertIn("skill.k", self.app)
        self.assertIn("skill.o", self.app)
        self.assertIn("...(skill.x || [])", self.app)
        self.assertIn('"workbuddy-ready": "WorkBuddy ready WorkBuddy 就绪 可直接使用"', self.app)
        self.assertIn('adaptable: "adaptable 可适配"', self.app)
        self.assertIn('"no-static-flags": "no static flags 无静态风险 无静态标记"', self.app)
        self.assertIn('"review-source": "review source 来源待审 分叉 镜像 休眠"', self.app)
        self.assertIn('skill.a ? (isChinese ? "精选包 可安装" : "reviewed package installable") : ""', self.app)
        ...
        self.assertIn('fetch("packages.json", requestOptions)', self.app)
        self.assertIn('const requestOptions = { cache: "no-cache", signal: controller.signal }', self.app)
        ...
        self.assertIn("copyReviewCommand", self.app)
        self.assertIn("copy-review", self.app)
        self.assertIn("--output review.md", self.app)
        self.assertIn("--language zh-CN", self.app)
        self.assertIn("Copy review command for ${escapeHtml(skill.n)}", self.app)
        self.assertIn("复制 ${escapeHtml(skill.n)} 的审阅命令", self.app)
        ...
        self.assertIn(".copy-review", self.styles)
